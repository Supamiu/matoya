...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addStringOption(option => option.setName('name').setDescription('Name of the card or part of it, or its code').setRequired(true))
        ...
        if (interaction.isSelectMenu()) {
            ...
            const matchingCards = cards.filter(c => {
                return getI18nProperty(c, 'name', lang).toLowerCase().indexOf(interaction.options.getString('name').toLowerCase()) > -1
                    ...
                    || c.code.toLowerCase() === interaction.options.getString('name').toLowerCase()
            });
